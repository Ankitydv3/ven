...
import { clearAllAlertsForUser, getAlertsCounts, getAlertsData } from "../services/alertsService";
import type { AuthRequest } from "../middleware/auth";
import { resolveTeamQuery, isTeamRole, taskVisibilityFilter } from "../utils/teamScope";

export async function getAlertsCount(req: AuthRequest, res: Response) {
  const teamOnly = isTeamRole(req.user?.role);
  const scopeFilter = taskVisibilityFilter(req.user);
  const counts = await getAlertsCounts({
    teamOnly,
