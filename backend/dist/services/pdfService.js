...
async function generateInvoicePDF(payment) {
    ...
    const drawText = (text, x, y, size = 10, fontType = font) => {
        page.drawText(text, { x, y, size, font: fontType, color: (0, pdf_lib_1.rgb)(0, 0, 0) });
    };
    // Header
    ...
    if (payment.complaintId)
        drawText(`Complaint ID: ${payment.complaintId}`, 50, height - 200);
    // Table Header
    const tableTop = height - 250;
    page.drawRectangle({ x: 50, y: tableTop - 5, width: width - 100, height: 20, color: (0, pdf_lib_1.rgb)(0.9, 0.9, 0.9) });
    ...
    if (payment.transactionId)
        drawText(`Transaction ID: ${payment.transactionId}`, 50, footerTop - 155);
