...
const COLUMN_ALIASES: Record<string, keyof OrderPayload | "paid" | "status" | "orderId"> = {
  "order id": "orderId",
  id: "orderId",
...
function mapRowToOrder(row: Record<string, unknown>, rowNumber: number) {
  const mapped: Partial<OrderPayload & { orderId?: string }> = {};
  ...
  for (const [header, rawValue] of Object.entries(row)) {
    ...
    if (key === "materialType") {
      ...
      mapped.orderId = String(rawValue).trim();
      ...
      mapped.salesPerson = String(rawValue).trim();
    }
  }

  const errors: string[] = [];
  if (!mapped.orderId) errors.push("Order ID is required");
  if (!mapped.customerName) errors.push("Name is required");
  if (!mapped.phone || mapped.phone.length !== 10) errors.push("Ph No must be 10 digits");
  if (!mapped.email) errors.push("Email is required");
  if (!mapped.materialType) errors.push("Material must be Aluminium or uPVC");
  if (!mapped.deliveryDate) errors.push("Handover Date is invalid");
...
export function downloadOrderImportTemplate() {
  ...
  const sampleRow = [
    ...
    "Aluminium",
    "2026-06-30",
    "123 MG Road, Gurgaon, Haryana 122001",
    "Rohan Gupta",
  ];
