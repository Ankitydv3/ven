...
import { buildActiveComplaintListParams, MY_TASKS_SCOPE } from "@/lib/active-complaints";
...
export function prefetchMyTasksList(queryClient: QueryClient) {
  const listParams = { scope: MY_TASKS_SCOPE, page: 1, limit: 10 };
...
export function prefetchDashboardRoute(
  ...
  if (href.includes(MY_TASKS_PATH)) {
...
export function prefetchCommonTeamRoutes(queryClient: QueryClient) {
  void prefetchMyTasksList(queryClient);
  void prefetchComplaintsList(queryClient, "team");
}
